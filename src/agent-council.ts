...
- 看到小赚 → "就赚 $5？均波 20pt 的行情你只吃 5 块钱？你在做慈善？"
...
function extractTradeContext(days: number = 14): string {
    ...
    // 当前策略参数
    ctx += `## 当前策略参数 (V200 五模组)\n`;
    ctx += `- 杠杆: 150x | 凯利动态仓位 (max 5ETH)\n`;
    ctx += `- 模组一: UTC+8 时间过滤 (4窗口: 09-10/15-16/20-22/22-24)\n`;
    ctx += `- 模组二: SVP POC 位移方向感知\n`;
    ctx += `- 模组三: 进场触发 (攻击日/引线回补/支撑测试)\n`;
    ctx += `- 模组四: 均波TP(H1均波×70%) + 3H时效律\n`;
    ctx += `- 模组五: 凯利风控 + 2%SL + 走三退一\n`;
